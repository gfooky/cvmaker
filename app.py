...
def load_crm() -> pd.DataFrame:
    ...
    if os.path.exists(CRM_FILE):
        ...
        # COMPATIBILITY: Updates the old CSV format to the new format automatically
        if "Status" in df.columns and "Job Status" not in df.columns:
            df.rename(columns={"Status": "Job Status"}, inplace=True)
        if "Job Status" not in df.columns:
            df["Job Status"] = "Not Sent"
        if "PDF Path" not in df.columns:
            df["PDF Path"] = ""
            
        # Clean possible empty values (NaN) generated by Pandas
        df["Job Status"] = df["Job Status"].fillna("Not Sent")
        df["PDF Path"] = df["PDF Path"].fillna("")
        
        return df
    
    return pd.DataFrame(
        columns=["Date", "Company", "Local Fit (%)", "AI Fit (%)", "Job Status", "Link/Note", "PDF Path"]
    )
...
def render_cv_and_save(
    ...
    pdf_path_to_save = ""  # Variable to store the path of the generated PDF
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if pdf_files:
                    ...
                    new_pdf_name = f"CV_{safe_company_name}_{timestamp}.pdf"
                    
                    output_dir = os.path.dirname(latest_pdf)
    ...
    # Save the new fields to the CSV
    save_to_crm(
        {
...
if selected_option == "➕ New Application":
    ...
    status_options = ["Not Sent", "Sent", "Interview", "Approved", "Rejected"]
    
    current_status = job_data.get('Job Status', 'Not Sent')
    if pd.isna(current_status) or current_status not in status_options:
        current_status = "Not Sent"
    ...
    # If the user changes the status in the menu, we update the CSV in real-time
    if new_status != current_status:
        df_crm.at[job_idx, 'Job Status'] = new_status
        df_crm.to_csv(CRM_FILE, index=False)
        st.toast(f"✅ Status updated to '{new_status}'!")
        st.rerun()  # Updates the screen to fix the change
    # -------------------------------------------
    ...
    # --- Display of Generated PDF ---
    pdf_path = job_data.get('PDF Path', '')
    
    if pd.notna(pdf_path) and str(pdf_path).strip() and os.path.exists(pdf_path):
        st.subheader("📄 Generated CV")
        
        with open(pdf_path, "rb") as f:
            st.download_button(
                label="⬇️ Download PDF again",
                data=f,
        ...
        st.info("💡 No PDF generated or file not found on disk for this application.")